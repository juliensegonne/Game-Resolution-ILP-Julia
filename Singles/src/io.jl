...
function read_matrix_from_file(file::String)
    ...
    matrix = Array{Int}(undef, nrows, ncols)
    
    # Remplir la matrice avec les données du fichier
    for (i, line) in enumerate(lines)
        row_data = parse.(Int, split(line, ","))
        matrix[i, :] = row_data
    end
...
function displayGrid(A::Matrix{Int64})
...
function displaySolution(x::JuMP.Containers.DenseAxisArray{VariableRef, 3})
